...
用户输入数字后，执行对应操作。操作完成后，**回到菜单**（除非用户选 0）。
...
先用 [1] 的格式展示全部条目（分页），提示用户输入条目序号（如 `#3`）：
...
```
这是哪类经验？
  [1] pitfall    — 踩过的坑（避免重蹈覆辙）
  [2] insight    — 发现的规律（某种方法特别有效）
  [3] correction — 修正了某个做法（之前做错了，现在知道怎么做对）
  [4] preference — 个人偏好（我就是这样喜欢的）
```
...
```
这条经验主要来自哪个步骤？
  [1] ppt-hours      [2] plan-mindmap   [3] plan-details
  [4] slide-content  [5] ppt-review     [6] gen-slides
  [0] 不确定/通用
```
...
提示：`写入 ~/.pptdog/learnings.jsonl？[Y/n]`
