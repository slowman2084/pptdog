...
PPT 正文不只是"不能写完整句子"，更要用正确的结构支撑标题论点。
对每个 Slide 的「PPT 上放」，检查是否选用了与论点类型匹配的内容结构：

| 论点类型 | 应有结构 | 常见缺陷 |
|---------|---------|---------|
| 数据型 | 改进前 vs 改进后对比数字 + 数据来源 | 只写一个数字，没有对比；数字没有来源 |
| 案例型 | 时间轴 / 冲突→解法两栏 + 关键转折词 | 只写"案例名称"或简述，无冲突感 |
| 方法论型 | 步骤图（动词短语）/ 框架图 | 步骤用名词（如"优化阶段"），缺动词；流程超过5步未分层 |
| 原理/因果型 | 因果链 / 公式 / 对比逻辑 | 只写结论，没有展示推导路径 |
| 架构/系统型 | 架构图 + 红框标注 + 聚焦模块说明 | 有架构图但无红框；试图讲整张图 |
| 对比型 | 两栏对比表 + 胜出维度高亮 | 只有我方数据，无对比项；胜出项不明显 |

若 Slide 正文只有关键词堆叠（≤3个词，无结构），且看不出论点支撑逻辑 → 标记为"内容单薄"。
