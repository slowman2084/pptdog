...
> - 若某个维度（空姐效应 / Why层 / 真实案例 / 假问题 / 授人以渔）审查后无问题，
>   才允许该维度为 0 条——但其他维度必须补充，确保总数 ≥ 5
> - 若 slide-content.md 内容充足（> 20 页），建议数应在 8-15 条之间
> - 不允许因为"内容看起来还不错"而少于 5 条——演讲永远有可以改的地方
>
> **自查提示（输出前 AI 必须做）：**
> 数一数当前建议总数，若 < 5 条：
>   → 回头重新扫描每个维度，找到之前遗漏的 minor 级问题
>   → 特别检查：每个论点的演讲口头说是否有至少一处可以口语化改进的地方
>   → 特别检查：是否有论点的 Why 层只有一层（应该有三层推导）
...
5. **context 字段（必填）**：用一句人话说清楚这条建议的背景
   - 格式：`"第N章·论点X.Y「[论点标题]」——[问题出在哪个部分]"`
   - 目的：让讲者在 Dashboard 里不用翻 slide-content.md 就能看懂这条建议
   - ❌ 错误：`"Slide 12–15 空姐效应"`
   - ✅ 正确：`"第2章·论点2.1「覆盖率从12%到99%」——PPT正文部分"`
6. **problem 字段必须说人话**（不用方法论术语）：
   - ❌ 不能写：`"空姐效应违规：PPT 正文包含大段陈述性文字"`
   - ✅ 要写：`"这页 PPT 上的字太多了，放映时听众会去读屏幕而不听你讲"`
   - ❌ 不能写：`"Why 层覆盖率不足，缺少因果推导链"`
   - ✅ 要写：`"你说了「要建立 SLO 体系」，但没解释为什么——听众会在心里问：为什么是 SLO 而不是别的？"`
7. **完整 suggestion 示例**：
   ```json
   {
     "id": "content-001",
     ...
     "dimension": "空姐效应",
     "severity": "major",
     "context": "第2章·论点2.1「覆盖率从12%到99%」——PPT正文部分",
     "source_lines": { "start": 45, "end": 52 },
     "original": "- 接入覆盖率从 12% 提升到 99%\n- 通过统一接入规范和强制检查机制\n- 分三个阶段推进，第一阶段覆盖核心链路...",
     "problem": "这页 PPT 上文字太多了，放映时听众会去读屏幕，不听你讲",
     "options": [
       {
         "label": "A",
         "description": "只留核心数字，原因留给演讲口头说",
         "content": "覆盖率：12% → 99%",
         "recommended": true,
         "reason": "数字对比一眼就记住，过程放在口头说里讲，不要让 PPT 抢戏"
       },
       {
         "label": "B",
         "description": "加一个进度条 / 前后对比布局",
         "content": "before_after 布局：左侧「接入前 12%」，右侧「接入后 99%」，中间箭头标注「+87pp」",
         "recommended": false,
         "reason": "视觉更直观，但需要换布局"
       },
       {
